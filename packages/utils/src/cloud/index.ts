...
export function getSecretManager() {
  ...
  console.log("\n[SecretManagerUtil] Secret manager type:", secretManagerType);

  switch (secretManagerType) {
    case "azure": {
      if (!process.env.AZURE_KEY_VAULT_URL) {
        throw new Error("Missing Azure Key Vault environment variables");
      }

      return new AzureKeyVaultSecretManager();
    }
    case "vault": {
      if (

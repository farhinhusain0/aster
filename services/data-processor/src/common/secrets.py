...
def fetch_secrets(secret_names: List[str]) -> Dict[str, str]:
    ...
    if secret_manager_type == "file":
        ...
        if (
            not os.getenv("AZURE_KEY_VAULT_URL")
            or not os.getenv("AZURE_CLIENT_ID")
            or not os.getenv("AZURE_TENANT_ID")
            or not os.getenv("AZURE_CLIENT_SECRET")
        ):
            raise Exception("Azure Key Vault URL, client ID, tenant ID, and client secret are required")
        return fetch_azure_secrets(secret_names)
    else:
...
def fetch_azure_secrets(secret_names: List[str]) -> Dict[str, str]:
    vault_url = os.getenv("AZURE_KEY_VAULT_URL")

    # Create a DefaultAzureCredential using the service principal credentials

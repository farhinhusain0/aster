...
async function init() {
  try {
    // Connect to DB first (this is needed for authorize function)
    // We only wait for the connection, not the seeding operations
    await connectAndInitDb(mongoUri, "slackbot");

    // Start the app immediately after DB connection
    ...
    await app.start();
    console.log(`\n[App] ⚡️ Slack Bolt app is running on port ${port}!`);
  } catch (error) {
    console.error("\n[App] Error starting the Slack Bolt app:", error);
  }
}

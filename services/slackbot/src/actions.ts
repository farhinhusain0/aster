...
export function attachCommands(app: App) {
  app.command("/aster", async ({ ack, say, command, client }) => {
    await ack();
    ...
    switch (command.text) {
      ...
      default:
        await say(
          "Invalid command 😕 You can run `/aster help` to see all the available commands",
        );
    }
  });

  // Listen for button click actions
  app.action("view_in_aster", async ({ ack, body, client }) => {
    await ack();
    console.log("view-in-aster action:", body);
  });
}
